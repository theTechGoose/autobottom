...
import { getFinding, saveFinding, getCachedAnswer, cacheAnswer, saveBatchAnswers, decrementBatchCounter, trackActive, getPopulatedQuestions } from "../../../../core/data/kv/impl.ts";
import { askQuestion } from "../../../../core/data/groq/impl.ts";
import { query as vectorQuery } from "../../../../core/data/pinecone/impl.ts";
import { parseAst } from "../../../../audit/domain/business/question-expr/impl.ts";
import { answerQuestion } from "../../../../audit/domain/business/question/impl.ts";
  ...
  for (const q of questions) {
    try {
      ...
      if (msg.includes("503") || msg.includes("over capacity") || msg.includes("429") || msg.toLowerCase().includes("rate limit")) {
        // Throw so the caller can handle retries

...
Deno.test("populateReviewQueue: populates queue for all-No answers", async () => {
  await withKv(async (kv) => {
    ...
    const item0 = await kv.db.get<ReviewItem>(Kv.orgKey(ORG, "review-pending", FINDING, 0));
    const item1 = await kv.db.get<ReviewItem>(Kv.orgKey(ORG, "review-pending", FINDING, 1));
...
Deno.test("populateReviewQueue: does nothing when all answers are Yes", async () => {
  await withKv(async (kv) => {
    ...
    for await (const entry of kv.db.list({ prefix: Kv.orgKey(ORG, "review-pending") })) {
      entries.push(entry);
    }
    assertEquals(entries.length, 0);

    // No review-audit-pending entry should exist
    const counter = await kv.db.get(Kv.orgKey(ORG, "review-audit-pending", FINDING));
...
Deno.test("populateReviewQueue: preserves original array index for each No answer", async () => {
  await withKv(async (kv) => {
    ...
    const skip0 = await kv.db.get<ReviewItem>(Kv.orgKey(ORG, "review-pending", FINDING, 0));
    const skip2 = await kv.db.get<ReviewItem>(Kv.orgKey(ORG, "review-pending", FINDING, 2));
    assertEquals(skip0.value, null);
    assertEquals(skip2.value, null);

    // Index 1 and 3 should exist with correct questionIndex
    const item1 = await kv.db.get<ReviewItem>(Kv.orgKey(ORG, "review-pending", FINDING, 1));
    const item3 = await kv.db.get<ReviewItem>(Kv.orgKey(ORG, "review-pending", FINDING, 3));
...
Deno.test("populateReviewQueue: sets review-audit-pending to count of No answers", async () => {
  await withKv(async (kv) => {
    ...
    const counter = await kv.db.get<number>(Kv.orgKey(ORG, "review-audit-pending", FINDING));
...
Deno.test("populateReviewQueue: ReviewItem stored in KV has correct shape", async () => {
  await withKv(async (kv) => {
    ...
    const entry = await kv.db.get<ReviewItem>(Kv.orgKey(ORG, "review-pending", FINDING, 0));
...
Deno.test("populateReviewQueue: only No items are queued in mixed array", async () => {
  await withKv(async (kv) => {
    ...
    for await (const entry of kv.db.list<ReviewItem>({ prefix: Kv.orgKey(ORG, "review-pending") })) {
...
Deno.test(
  ...
  async () => {
    await withKv(async (kv) => {
      ...
      const lockEntry = await kv.db.get<{ claimedBy: string; claimedAt: number }>(
        Kv.orgKey(ORG_INT, "review-lock", finding, claimedIdx),
      );
...
Deno.test(
  ...
  async () => {
    await withKv(async (kv) => {
      ...
      const decidedEntry = await kv.db.get<ReviewDecision>(
        Kv.orgKey(ORG_INT, "review-decided", finding, qi),
      );
      ...
      const pendingEntry = await kv.db.get(Kv.orgKey(ORG_INT, "review-pending", finding, qi));

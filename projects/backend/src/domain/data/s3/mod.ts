/**
 * Deploy-compatible S3 using native fetch + AWS Signature V4.
 * Avoids npm:@aws-sdk/client-s3 which has integer overflow issues on Deno Deploy.
 */
...
function getAccessKey(): string {
  return Deno.env.get("AWS_ACCESS_KEY_ID")!;
}
function getSecretKey(): string {
  return Deno.env.get("AWS_SECRET_ACCESS_KEY")!;
}
...
async function signV4(method: string, bucket: string, key: string, payloadHash: string, headers: Record<string, string>) {
  ...
  let signingKey: ArrayBuffer = await hmac(new TextEncoder().encode("AWS4" + secretKey), dateStamp);
...
export class S3Ref {
  ...
  async get(): Promise<Uint8Array | null> {
    ...
    if (!res.ok) {
      const text = await res.text();
      // Check for NoSuchKey in S3 XML response

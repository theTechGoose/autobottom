...
export async function handleAuditByRid(orgId: OrgId, req: Request): Promise<Response> {
  ...
  await (await kv()).saveJob(orgId, job);
...
export async function handleGetFinding(orgId: OrgId, req: Request): Promise<Response> {
  ...
  const finding = await (await kv()).getFinding(orgId, id);
...
export async function handleGetStats(orgId: OrgId, _req: Request): Promise<Response> {
  const stats = await (await kv()).getStats(orgId);
...
export async function handleGetRecording(orgId: OrgId, req: Request): Promise<Response> {
  ...
  const s3 = new S3Ref(Deno.env.get("S3_BUCKET")!, recordingPath);
...
export async function handleFileAppeal(orgId: OrgId, req: Request): Promise<Response> {
  ...
  const finding = await (await kv()).getFinding(orgId, findingId);
  if (!finding) return json({ error: "finding not found" }, 404);

  const f = finding as Record<string, any>;
  const allAnswers = await (await kv()).getAllAnswersForFinding(orgId, findingId);
  ...
  (await kv()).fireWebhook(orgId, "appeal", {
    findingId,
    finding: f,
    auditor: f.owner,
...
export async function handleAppealDifferentRecording(orgId: OrgId, req: Request): Promise<Response> {
  ...
  const oldFinding = await (await kv()).getFinding(orgId, findingId);
  ...
  await (await kv()).saveFinding(orgId, oldFinding as Record<string, any>);
  ...
  await (await kv()).saveJob(orgId, newJob);
  ...
  await (await kv()).saveFinding(orgId, newFinding as Record<string, any>);
  await enqueueStep("init", { findingId: newFindingId });

  const reportUrl = `${Deno.env.get("SELF_URL")!}/audit/report?id=${newFindingId}`;
...
export async function handleAppealUploadRecording(orgId: OrgId, req: Request): Promise<Response> {
  ...
  const s3 = new S3Ref(Deno.env.get("S3_BUCKET")!, s3Key);

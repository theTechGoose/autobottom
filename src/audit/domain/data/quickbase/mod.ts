...
// ── Reservations / MCC ID lookup (used by email reports) ────────────────────

const RESERVATIONS_TABLE = "bmhvhc72c";
const RES = { RECORD_ID: 3, MOST_RECENT_ACTIVE_MCC: 991 };
...
export async function getReservationRidsForDateLegs(rids: string[]): Promise<Map<string, string>> {
  ...
  const records = await queryRecords({
    tableId: DATE_LEGS_TABLE,
    where: buildRidOrWhere(DL.RECORD_ID, rids),
    select: [DL.RECORD_ID, FIELD_DATELEG_RELATED_RESERVATION],
  });
  for (const r of records) {
    const childRid = String(r[DL.RECORD_ID]?.value ?? "");
...
export async function getReservationRidsForPackages(rids: string[]): Promise<Map<string, string>> {
  ...
  const records = await queryRecords({
    tableId: PACKAGES_TABLE,
    where: buildRidOrWhere(PK.RECORD_ID, rids),
    select: [PK.RECORD_ID, FIELD_PACKAGE_RELATED_RESERVATION],
  });
  for (const r of records) {
    const childRid = String(r[PK.RECORD_ID]?.value ?? "");
...
export async function getMccIdsForReservations(rids: string[]): Promise<Map<string, string>> {
  ...
  const records = await queryRecords({
    ...
    where: buildRidOrWhere(RES.RECORD_ID, rids),
    select: [RES.RECORD_ID, RES.MOST_RECENT_ACTIVE_MCC],
  });
  for (const r of records) {
    const resRid = String(r[RES.RECORD_ID]?.value ?? "");
    const mcc = String(r[RES.MOST_RECENT_ACTIVE_MCC]?.value ?? "");

  ...
  return async (req: Request): Promise<Response> => {
    ...
    try {
      ...
      const pipelineCfg = retryOrgId
        ? await (await Kv.getInstance()).getPipelineConfig(retryOrgId)
      ...
      if (retryOrgId) {
        (await Kv.getInstance()).trackError(retryOrgId, fid, stepName, msg).catch(() => {});
      }
      ...
      if (attempt <= pipelineCfg.maxRetries) {
        ...
        if (retryOrgId) {
          (await Kv.getInstance()).trackRetry(retryOrgId, fid, stepName, attempt).catch(() => {});
        }
        try {
      ...
      if (retryOrgId) {
        (await Kv.getInstance()).trackCompleted(retryOrgId, fid).catch(() => {});
      }
      sendEmail({
...
<p><a href="${Deno.env.get("SELF_URL")!}/audit/report?id=${fid}">View Report</a></p>`,

...
export class StoreService {
  rarityFromPrice(price: number): StoreRarity {
  ...
  getStoreCatalogJson(): string {
    return JSON.stringify(STORE_CATALOG);
  }
}

// Old API preserved as wrappers
const _svc = new StoreService();
export function rarityFromPrice(
  ...args: Parameters<StoreService["rarityFromPrice"]>
): StoreRarity {
  return _svc.rarityFromPrice(...args);
}
export function getStoreCatalogJson(
  ...args: Parameters<StoreService["getStoreCatalogJson"]>
): string {
  return _svc.getStoreCatalogJson(...args);
}

...
import type { WebhookConfig, WebhookKind, GamificationSettings, OrgId } from "../domain/data/kv/mod.ts";
...
async function kv() { return await Kv.getInstance(); }
...
export async function handleBadgeEditorItems(req: Request): Promise<Response> {
  const auth = await requireAdminAuth(req);
  if (auth instanceof Response) return auth;
  const custom = await (await kv()).listCustomStoreItems(auth.orgId);
...
export async function handleBadgeEditorSave(req: Request): Promise<Response> {
  ...
  await (await kv()).saveCustomStoreItem(auth.orgId, item);
...
export async function handleBadgeEditorDelete(req: Request): Promise<Response> {
  ...
  await (await kv()).deleteCustomStoreItem(auth.orgId, id);
  return json({ ok: true });
}
...
export async function handleAdminGetSettings(req: Request): Promise<Response> {
  ...
  const config = await (await kv()).getWebhookConfig(auth.orgId, kind);
...
export async function handleAdminSaveSettings(req: Request): Promise<Response> {
  ...
  await (await kv()).saveWebhookConfig(auth.orgId, kind, config);
  return json({ ok: true });
}
...
export async function handleGetPipelineConfig(req: Request): Promise<Response> {
  ...
  return json(await (await kv()).getPipelineConfig(auth.orgId));
}
...
export async function handleSetPipelineConfig(req: Request): Promise<Response> {
  ...
  const result = await (await kv()).setPipelineConfig(auth.orgId, update);
  return json(result);
}
...
export async function handleAdminGetGamification(req: Request): Promise<Response> {
  const auth = await requireAdminAuth(req);
  if (auth instanceof Response) return auth;
  const settings = await (await kv()).getGamificationSettings(auth.orgId);
...
export async function handleAdminSaveGamification(req: Request): Promise<Response> {
  ...
  await (await kv()).saveGamificationSettings(auth.orgId, body);
  return json({ ok: true });
}
...
export async function handleGetQueues(_req: Request): Promise<Response> {
  const res = await fetch(`${Deno.env.get("QSTASH_URL")!}/v2/queues`, {
    headers: { Authorization: `Bearer ${Deno.env.get("QSTASH_TOKEN")!}` },
  });
  if (!res.ok) {
...
export async function handleGetParallelism(_req: Request): Promise<Response> {
  const res = await fetch(`${Deno.env.get("QSTASH_URL")!}/v2/queues/${QUEUE_NAME}`, {
    headers: { Authorization: `Bearer ${Deno.env.get("QSTASH_TOKEN")!}` },
  });
  if (!res.ok) {
    const text = await res.text();
...
export async function handleListEmailReports(req: Request): Promise<Response> {
  ...
  const configs = await (await kv()).listEmailReportConfigs(auth.orgId);
  return json(configs);
}
...
export async function handleSaveEmailReport(req: Request): Promise<Response> {
  ...
  const saved = await (await kv()).saveEmailReportConfig(auth.orgId, body);
  return json(saved);
}
...
export async function handleDeleteEmailReport(req: Request): Promise<Response> {
  ...
  await (await kv()).deleteEmailReportConfig(auth.orgId, body.id);
  return json({ ok: true });
}
...
export async function handleForceNos(req: Request): Promise<Response> {
  ...
  await (await kv()).saveFinding(auth.orgId, finding);
...
export async function seedOrgData(orgId: OrgId): Promise<{ seeded: number; managerSeeded: number; judgeSeeded: number; qlabSeeded: number; orgId: OrgId }> {
  ...
  for (const finding of findings) {
    ...
    await (await kv()).saveFinding(orgId, finding);

    if (finding.rawTranscript) {
      await (await kv()).saveTranscript(orgId, finding.id, finding.rawTranscript, finding.diarizedTranscript);
    }

    if (finding.answeredQuestions?.length) {
      await (await kv()).saveBatchAnswers(orgId, finding.id, 0, finding.answeredQuestions);
    }
    ...
    await (await kv()).trackCompleted(orgId, finding.id);
  ...
  for (let i = 0; i < Math.min(12, findings.length); i++) {
    ...
    for (const q of noQuestions) {
      await db.delete(Kv.orgKey(orgId, "review-pending", finding.id, q.idx));
    }
    await db.delete(Kv.orgKey(orgId, "review-audit-pending", finding.id));
    ...
    for (const q of noQuestions) {
      ...
      await db.set(Kv.orgKey(orgId, "review-decided", finding.id, q.idx), {
    ...
    const queueEntry = await db.get(Kv.orgKey(orgId, "manager-queue", finding.id));
    if (queueEntry.value) {
      const updated = { ...(queueEntry.value as Record<string, any>), completedAt };
      await db.set(Kv.orgKey(orgId, "manager-queue", finding.id), updated);
    }
    ...
    if (i < 6) {
      ...
      const remEntry = await db.get(Kv.orgKey(orgId, "manager-remediation", finding.id));
      if (remEntry.value) {
        const updated = { ...(remEntry.value as Record<string, any>), addressedAt: remediatedAt };
        await db.set(Kv.orgKey(orgId, "manager-remediation", finding.id), updated);
      }
    }
  ...
  for (const [email, purchases, xp, tokens] of cosmeticProfiles) {
    const existing = await (await kv()).getGameState(orgId, email);
    await (await kv()).saveGameState(orgId, email, {
      ...existing,
...
export async function handleResetFinding(req: Request): Promise<Response> {
  ...
  const listPrefixes = [
    Kv.orgKey(auth.orgId, "review-pending", findingId),
    Kv.orgKey(auth.orgId, "review-decided", findingId),
    Kv.orgKey(auth.orgId, "review-lock", findingId),
    Kv.orgKey(auth.orgId, "judge-pending", findingId),
    Kv.orgKey(auth.orgId, "judge-decided", findingId),
    Kv.orgKey(auth.orgId, "judge-lock", findingId),
  ];
  ...
  const exactKeys: Deno.KvKey[] = [
    Kv.orgKey(auth.orgId, "review-audit-pending", findingId),
    Kv.orgKey(auth.orgId, "judge-audit-pending", findingId),
    Kv.orgKey(auth.orgId, "appeal", findingId),
    Kv.orgKey(auth.orgId, "manager-queue", findingId),
    Kv.orgKey(auth.orgId, "manager-remediation", findingId),
  ];
  ...
  let queued = 0;
  const answers = await (await kv()).getAllAnswersForFinding(auth.orgId, findingId);
  if (answers?.length) {

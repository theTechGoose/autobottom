...
export async function stepFinalize(req: Request): Promise<Response> {
  ...
  if (totalBatches && totalBatches > 0) {
    const allAnswers = await kv.getAllBatchAnswers(orgId, findingId, totalBatches);
  ...
  if (isInvalid && finding.populatedQuestions?.length) {
    ...
    finding.feedback = {
      ...
      viewUrl: "",
      disputeUrl: `${Deno.env.get("SELF_URL")!}/audit/appeal?findingId=${findingId}`,
      recordingUrl: `${Deno.env.get("SELF_URL")!}/audit/recording?id=${findingId}`,
    };
  }
    ...
    try {
      ...
      if (failedQs) {
        ...
        finding.feedback = {
          ...
          text: feedbackText,
          viewUrl: `${Deno.env.get("DENO_KV_URL")!}/get?id=${findingId}`,
          disputeUrl: `${Deno.env.get("SELF_URL")!}/audit/appeal?findingId=${findingId}`,
          recordingUrl: `${Deno.env.get("SELF_URL")!}/audit/recording?id=${findingId}`,
        };
      }
  ...
  await kv.trackCompleted(orgId, findingId);
  ...
  if (finding.owner) {
    const score = finding.answeredQuestions?.length
      ...
      : 0;
    kv.emitEvent(orgId, finding.owner, "audit-completed", {
      findingId,
      score,
  ...
  if (finding.owner && finding.answeredQuestions?.length) {
    try {
      ...
      // Update badge stats
      const stats = await kv.getBadgeStats(orgId, finding.owner);
      stats.totalAudits++;
      ...
      await kv.updateBadgeStats(orgId, finding.owner, stats);

      const earned = await kv.getEarnedBadges(orgId, finding.owner);
      const earnedSet = new Set(earned.map((b) => b.badgeId));
      const newBadges = checkBadges("agent", stats, earnedSet);

      let badgeXp = 0;
      for (const badge of newBadges) {
        await kv.awardBadge(orgId, finding.owner, badge);
        badgeXp += badge.xpReward;
      }

      const awardResult = await kv.awardXp(orgId, finding.owner, xp + badgeXp, "agent");
      if (newBadges.length) {
        console.log(`[STEP-FINALIZE] ${findingId}: Agent ${finding.owner} earned ${newBadges.length} badge(s)`);
        for (const badge of newBadges) {
          kv.checkAndEmitPrefab(orgId, "badge_earned", finding.owner, `${finding.owner.split("@")[0]} earned ${badge.name}!`)
            .catch(() => {});
        }
      }

      // Broadcast: sale_completed
      kv.checkAndEmitPrefab(orgId, "sale_completed", finding.owner, `${finding.owner.split("@")[0]} completed an audit!`)
        .catch(() => {});

      // Broadcast: perfect_score
      if (score === 100) {
        kv.checkAndEmitPrefab(orgId, "perfect_score", finding.owner, `${finding.owner.split("@")[0]} got a perfect score!`)
          .catch(() => {});
      }
      ...
      if (awardResult.state.dayStreak === 7 || awardResult.state.dayStreak === 14 || awardResult.state.dayStreak === 30) {
        kv.checkAndEmitPrefab(orgId, "streak_milestone", finding.owner, `${finding.owner.split("@")[0]} hit a ${awardResult.state.dayStreak}-day streak!`)
          .catch(() => {});
      }
    } catch (err) {
  ...
  if (finding.auditJobId) {
    try {
      const job = await kv.getJob(orgId, finding.auditJobId);
      if (job) {
  ...
  if (nos === 0 && yeses > 0 && !isAppealReAudit) {
    kv.fireWebhook(orgId, "terminate", {
      findingId,
      finding,
...
async function postToDeno(finding: Record<string, any>) {
  const DENO_URL = Deno.env.get("DENO_KV_URL")!;

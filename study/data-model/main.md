...
| **Audit Template** | AuditQuestion | belongs to AuditConfig, has QuestionTests, references QuestionType (enum) |
...
A question template belonging to a config. The question text is a field
(`template`) on AuditQuestion, not a separate object. Each question has an
`expectedAnswer` and a `questionType` that defines and enforces the answer
format. Skip logic lives on the AuditConfig, not here.

```typescript
// QuestionType is an enum -- a predefined set of all available question
// type names. Each entry carries its own regex and parser key.

type QuestionParser = "bool" | "int" | "float" | "string";

interface QuestionTypeDef {
  name: string;                    // Type name (the enum key)
  regex: string;                   // Validation pattern for accepted input
  parser: QuestionParser;          // Key to runtime parser function
  schema: object;                  // JSON Schema representation of this type's value
}                                  // e.g. { type: "string" }, { type: "boolean" }, { type: "integer" }

// Runtime parser registry (not stored, lives in code):
// bool:   (v) => /^(yes|true|1|pass)$/i.test(v)
// int:    (v) => parseInt(v, 10)
...
interface AuditQuestion {
  ...
  template: string;                // Question text with placeholders (not a separate object)
  expectedAnswer: string;          // The expected answer for this question
  questionType: string;            // References a QuestionTypeDef name (enum)

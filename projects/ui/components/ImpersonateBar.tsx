/**
 * ImpersonateBar — server-rendered component showing the impersonation banner.
 * Renders a thin amber line at top that expands to full bar on hover.
 * Loads users via /admin/users and patches fetch to append ?as=email on same-origin calls.
 */

interface ImpersonateBarProps {
  targetRole: string;
  currentAsEmail: string;
}

export function ImpersonateBar({ targetRole, currentAsEmail }: ImpersonateBarProps) {
  const currentAsJson = JSON.stringify(currentAsEmail);
  const targetRoleJson = JSON.stringify(targetRole);
...
`,
      ...
      <div id="impersonate-bar">
        <span style="font-weight:700;letter-spacing:0.5px;text-transform:uppercase;font-size:9px;color:#fbbf24;flex-shrink:0;">
          ADMIN VIEW
        </span>
        <span style="color:#fde68a;flex-shrink:0;">Viewing as:</span>
        <select
          id="impersonate-select"
          style="padding:1px 6px;border-radius:4px;border:1px solid #f59e0b;background:#451a03;color:#fef3c7;font-size:10px;font-family:inherit;cursor:pointer;"
        >
        ...
        <a
          id="impersonate-exit"
          href="#"
          style="margin-left:auto;color:#fbbf24;font-size:10px;font-weight:600;text-decoration:underline;flex-shrink:0;"
        >
          Exit Impersonation
        </a>
      </div>
      <script
        dangerouslySetInnerHTML={{
          __html: `
(function(){
  var currentAs = ${currentAsJson};
  var targetRole = ${targetRoleJson};

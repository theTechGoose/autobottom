...
Deno.test("populateJudgeQueue: stores all questions including Yes answers", async () => {
  await withKv(async (kv) => {
    ...
    for await (const entry of kv.db.list<JudgeItem>({ prefix: Kv.orgKey(ORG, "judge-pending", finding) })) {
      items.push(entry.value);
    }
    assertEquals(items.length, 3, "all 3 questions should be queued");

    // Counter should be 3
    const counter = await kv.db.get<number>(Kv.orgKey(ORG, "judge-audit-pending", finding));
...
Deno.test("populateJudgeQueue: stored item has correct shape", async () => {
  await withKv(async (kv) => {
    ...
    const entry = await kv.db.get<JudgeItem>(Kv.orgKey(ORG, "judge-pending", finding, 0));
...
Deno.test(
  ...
  async () => {
    await withKv(async (kv) => {
      ...
      const lockEntry = await kv.db.get<{ claimedBy: string }>(
        Kv.orgKey(ORG, "judge-lock", finding, qi),
      );
...
Deno.test(
  ...
  async () => {
    await withKv(async (kv) => {
      ...
      const decidedEntry = await kv.db.get<JudgeDecision>(
        Kv.orgKey(ORG, "judge-decided", finding, qi),
      );
      ...
      const pendingEntry = await kv.db.get(Kv.orgKey(ORG, "judge-pending", finding, qi));

  ...
  useEffect(() => {
    ...
    async function submitConfirm() {
      ...
      try {
        ...
        if (finalAnswer) {
          // Forward the ReviewTiming island's idle flag for this final question
          // (its htmx:configRequest hook can't fire on this manual fetch). Duration
          // is measured server-side from decision timestamps.
          const timing = (globalThis as unknown as { __reviewTiming?: () => { idleMs: number } })
            .__reviewTiming?.() ?? { idleMs: 0 };

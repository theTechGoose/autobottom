...
import { populateManagerQueue } from "../../../../manager/domain/coordinators/manager/impl.ts";
import { checkBadges, BADGE_CATALOG } from "../../../../core/business/badges/mod.ts";
...
export async function populateReviewQueue(
  ...
  for (const q of noAnswers) {
    ...
    atomic.set(Kv.orgKey(orgId, "review-pending", findingId, q.index), item);
  }
  atomic.set(Kv.orgKey(orgId, "review-audit-pending", findingId), noAnswers.length);
  await atomic.commit();
...
export async function claimNextItem(orgId: OrgId, reviewer: string): Promise<{
  ...
  const iter = db.list<ReviewItem>({ prefix: Kv.orgKey(orgId, "review-pending") });
  for await (const entry of iter) {
    remaining++;
    const item = entry.value;
    const lockKey = Kv.orgKey(orgId, "review-lock", item.findingId, item.questionIndex);
  ...
  if (current) {
    ...
    const counterEntry = await db.get<number>(Kv.orgKey(orgId, "review-audit-pending", current.findingId));
...
export async function recordDecision(
  ...
  const lockKey = Kv.orgKey(orgId, "review-lock", findingId, questionIndex);
  ...
  // Load pending item
  const pendingKey = Kv.orgKey(orgId, "review-pending", findingId, questionIndex);
  ...
  const counterKey = Kv.orgKey(orgId, "review-audit-pending", findingId);
  ...
  const atomic = db.atomic()
    ...
    .delete(lockKey)
    .delete(pendingKey)
    .set(Kv.orgKey(orgId, "review-decided", findingId, questionIndex), decided);
  ...
  try {
    const stats = await kvInst.getBadgeStats(orgId, reviewer);
    ...
    await kvInst.updateBadgeStats(orgId, reviewer, stats);

    const earned = await kvInst.getEarnedBadges(orgId, reviewer);
    const earnedSet = new Set(earned.map((b) => b.badgeId));
    newBadges = checkBadges("reviewer", stats, earnedSet);

    let badgeXp = 0;
    for (const badge of newBadges) {
      await kvInst.awardBadge(orgId, reviewer, badge);
      badgeXp += badge.xpReward;
    }
    // Award base XP (10 per decision) + badge bonus XP
    await kvInst.awardXp(orgId, reviewer, 10 + badgeXp, "reviewer");
  } catch (err) {
...
export async function undoDecision(
  ...
  const lockIter = db.list<{ claimedBy: string }>({ prefix: Kv.orgKey(orgId, "review-lock") });
  ...
  const atomic = db.atomic()
    ...
    .set(Kv.orgKey(orgId, "review-pending", findingId, questionIndex), item)
    .set(counterKey, newCount)
    .set(
      Kv.orgKey(orgId, "review-lock", findingId, questionIndex),
  ...
  let peek: ReviewItem | null = null;
  let remaining = 0;
  const pendingIter = db.list<ReviewItem>({ prefix: Kv.orgKey(orgId, "review-pending") });
  for await (const entry of pendingIter) {
    remaining++;
    if (!peek && !(entry.value.findingId === findingId && entry.value.questionIndex === questionIndex)) {
      const lk = Kv.orgKey(orgId, "review-lock", entry.value.findingId, entry.value.questionIndex);
...
async function postCorrectedAudit(orgId: OrgId, findingId: string) {
  ...
  await kvInst.fireWebhook(orgId, "terminate", {
    findingId,
...
export async function getReviewStats(orgId: OrgId): Promise<{ pending: number; decided: number }> {
  ...
  for await (const _ of db.list({ prefix: Kv.orgKey(orgId, "review-pending") })) pending++;
  for await (const _ of db.list({ prefix: Kv.orgKey(orgId, "review-decided") })) decided++;
...
export async function getReviewerDashboardData(orgId: OrgId, reviewer: string): Promise<ReviewerDashboardData> {
  ...
  const iter = db.list<ReviewDecision>({ prefix: Kv.orgKey(orgId, "review-decided") });
  for await (const entry of iter) {
    decided++;
    const d = entry.value;
...
export async function backfillFromFinished(orgId: OrgId) {
  ...
  // Collect unique finding IDs from chunked KV keys (pattern: Kv.orgKey(orgId, "audit-finding", id, chunkIndex))
  ...
  for (const findingId of findingIds) {
    ...
    const existingCheck = await db.get(Kv.orgKey(orgId, "review-audit-pending", findingId));
    if (existingCheck.value !== null) continue;

    // Also check if any decided items exist
    let hasDecided = false;
    const decidedIter = db.list({ prefix: Kv.orgKey(orgId, "review-decided", findingId) });

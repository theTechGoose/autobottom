...
async function getConfigIndex(orgId: OrgId): Promise<string[]> {
  const db = await kv();
  const entry = await db.get<string[]>(Kv.orgKey(orgId, "qlab", "config-index"));
  return entry.value ?? [];
}

async function setConfigIndex(orgId: OrgId, ids: string[]) {
  const db = await kv();
  await db.set(Kv.orgKey(orgId, "qlab", "config-index"), ids);
}
...
export async function getConfig(orgId: OrgId, id: string): Promise<QLConfig | null> {
  const db = await kv();
  const entry = await db.get<QLConfig>(Kv.orgKey(orgId, "qlab", "config", id));
...
export async function createConfig(orgId: OrgId, name: string): Promise<QLConfig> {
  ...
  const db = await kv();
  await db.set(Kv.orgKey(orgId, "qlab", "config", id), config);
...
export async function deleteConfig(orgId: OrgId, id: string): Promise<void> {
  ...
  const db = await kv();
  await db.delete(Kv.orgKey(orgId, "qlab", "config", id));
  const index = await getConfigIndex(orgId);
...
export async function getQuestion(orgId: OrgId, id: string): Promise<QLQuestion | null> {
  const db = await kv();
  const entry = await db.get<QLQuestion>(Kv.orgKey(orgId, "qlab", "question", id));
  return entry.value ?? null;
}
...
export async function createQuestion(orgId: OrgId, configId: string, name: string, text: string): Promise<QLQuestion | null> {
  ...
  const db = await kv();
  await db.set(Kv.orgKey(orgId, "qlab", "question", id), question);
  config.questionIds.push(id);
  await db.set(Kv.orgKey(orgId, "qlab", "config", configId), config);
  return question;
}
...
export async function deleteQuestion(orgId: OrgId, id: string): Promise<void> {
  ...
  const db = await kv();
  await db.delete(Kv.orgKey(orgId, "qlab", "question", id));
  const config = await getConfig(orgId, question.configId);
  if (config) {
    config.questionIds = config.questionIds.filter((qId) => qId !== id);
    await db.set(Kv.orgKey(orgId, "qlab", "config", question.configId), config);
  }
}
...
export async function getTest(orgId: OrgId, id: string): Promise<QLTest | null> {
  const db = await kv();
  const entry = await db.get<QLTest>(Kv.orgKey(orgId, "qlab", "test", id));
  return entry.value ?? null;
}
...
export async function createTest(
  ...
  const db = await kv();
  await db.set(Kv.orgKey(orgId, "qlab", "test", id), test);
  question.testIds.push(id);
  await db.set(Kv.orgKey(orgId, "qlab", "question", questionId), question);
  return test;
}
...
export async function deleteTest(orgId: OrgId, id: string): Promise<void> {
  ...
  if (!test) return;
  const db = await kv();
  await db.delete(Kv.orgKey(orgId, "qlab", "test", id));
  const question = await getQuestion(orgId, test.questionId);
  if (question) {
    question.testIds = question.testIds.filter((tId) => tId !== id);
    await db.set(Kv.orgKey(orgId, "qlab", "question", test.questionId), question);
  }
}

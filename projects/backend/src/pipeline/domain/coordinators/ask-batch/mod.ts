...
import { Kv } from "../../../../core/domain/data/kv/mod.ts";
import { enqueueStep } from "../../../../core/domain/data/queue/mod.ts";
import { askQuestion } from "../../../../core/domain/data/groq/mod.ts";
import { query as vectorQuery } from "../../../../core/domain/data/pinecone/mod.ts";
import { parseAst } from "../../../../core/domain/business/question-expr/mod.ts";
import { answerQuestion } from "../../../../core/domain/business/question/mod.ts";
import type { IQuestion, IAnsweredQuestion, ILlmQuestionAnswer } from "../../../../core/dto/question.ts";
...
export function strToBool(s: string): boolean | null {
...
async function askLlmOne(
  ...
  // Check cache
  const cached = await (await Kv.getInstance()).getCachedAnswer(orgId, findingId, question.populated);
  ...
  if (ast.length === 0 || (ast.length === 1 && ast[0].length === 1 && !ast[0][0].flip)) {
    ...
    await (await Kv.getInstance()).cacheAnswer(orgId, findingId, question.populated, answer);
  ...
  for (const andNodes of ast) {
    ...
    for (const node of andNodes) {
      ...
      if (boolAnswer === null) {
        ...
        await (await Kv.getInstance()).cacheAnswer(orgId, findingId, question.populated, fallbackAnswer);
  ...
  await (await Kv.getInstance()).cacheAnswer(orgId, findingId, question.populated, finalAnswer);
...
export async function stepAskBatch(req: Request): Promise<Response> {
  ...
  (await Kv.getInstance()).trackActive(orgId, findingId, `ask-batch-${batchIndex}`).catch(() => {});

  const finding = await (await Kv.getInstance()).getFinding(orgId, findingId);
  if (!finding) return json({ error: "finding not found" }, 404);

  // Read from dedicated chunked KV key first (survives finding trim), fall back to finding
  const allPopulated = await (await Kv.getInstance()).getPopulatedQuestions(orgId, findingId) ?? finding.populatedQuestions ?? [];
  ...
  // Save batch answers
  await (await Kv.getInstance()).saveBatchAnswers(orgId, findingId, batchIndex, answers);
  console.log(`[STEP-ASK] ${findingId}: Batch ${batchIndex} done (${answers.length} answers)`);

  // Fan-in: decrement counter
  const remaining = await (await Kv.getInstance()).decrementBatchCounter(orgId, findingId);

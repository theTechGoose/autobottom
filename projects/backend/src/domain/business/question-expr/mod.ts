...
export class QuestionExprService {
  ...
  parseAst(question: IQuestion): IQuestion {
    ...
    const { cleaned, notes } = this.pullNotes(question.populated);
  ...
  /** Extract backtick-fenced notes from a string. */
  pullNotes(input: string): { cleaned: string; notes: string[] } {
    const notes: string[] = [];
...
const _service = new QuestionExprService();
...
export function populateQuestions(
  ...
  return _service.populateQuestions(questions, record, fieldLookup);
}

/** Parse compound question AST (|, &, !, +: operators). */
export function parseAst(question: IQuestion): IQuestion {
  return _service.parseAst(question);
}

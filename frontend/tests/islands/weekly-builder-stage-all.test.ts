 ...
 *  exclusions, and the skip-already-staged/published behavior. */
...
Deno.test("planStageAll internal — a specific shift is applied; 'All' means no shift", () => {
  const base = {
  ...
  assertEquals(planStageAll({ ...base, type: "internal", shift: "AM" })[0].shift, "AM");
  assertEquals(planStageAll({ ...base, type: "internal", shift: "All" })[0].shift, null);
});
...
Deno.test("planStageAll — skips anything already staged or published", () => {
  const plan = planStageAll({
    type: "internal", shift: "All",
...
Deno.test("planStageAll partner — excluded office is skipped", () => {
  const plan = planStageAll({
    type: "partner", shift: "All",
  ...
  assertEquals(plan.map((p) => p.office), ["EAST"]);
});

/**
 * ReviewQueue island — interactive review queue with keyboard shortcuts,
 * transcript display, audio player, combo meter, and gamification.
 *
 * Fetches from /api/review/next, submits to /api/review/decide.
 * Hotkeys: Y=confirm, N=flip, D=toggle reasoning, B=undo, H/L=scroll columns,
...
export default function ReviewQueue() {
  ...
  // Confetti colors (review = purple/blue)
  const confettiColors = ["#8b5cf6", "#1f6feb", "#58a6ff", "#bc8cff", "#fab005"];
  ...
  function getGameState() {
    try {
      return JSON.parse(localStorage.getItem(`${STORAGE_PREFIX}_game_${reviewer.value}`) || "{}");
    } catch { return {}; }
  }
  ...
  function resetCombo() {
    ...
    combo.value = 0;
    timeBankVal.value = 0;
    if (timeBankAnimRef.current) {
      cancelAnimationFrame(timeBankAnimRef.current);
      timeBankAnimRef.current = null;
    }
  }
    ...
    for (const line of lines) {
      ...
      if (match) {
        ...
        if (isEvidence) {
          ...
          if (matchCount >= 3) cls += " t-highlight";
        }
  ...
  function skipSeek(dir: number) {
    ...
    if (elapsed < 1000 && skipTier.current < SKIP_TIERS.length - 1) {
      skipTier.current++;
  ...
  async function executeDecision(decision: string, reason?: string) {
    ...
    toast(decision === "confirm" ? "Confirmed fail" : "Flipped to pass", decision);
    ...
    try {
      ...
      if (data.auditComplete) toast("Audit complete", "complete");
  ...
  useEffect(() => {
    function handleKey(e: KeyboardEvent) {
      ...
      switch (e.key.toLowerCase()) {
        ...
        case "d": thinkingOpen.value = !thinkingOpen.value; break;
        case "b":
  ...
  function runSearch(q: string) {
    const lines = document.querySelectorAll("#transcript-body .t-line");
    ...
    lines.forEach((el) => {
      if (el.textContent?.toLowerCase().includes(lq)) {
        el.classList.add("t-search-match");
        matches.push(el);
      }
    });
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const data = await res.json();
        reviewer.value = data.reviewer || "reviewer";
        loadGameState();
        ...
        try {
          ...
          if (gcRes.ok) {
            ...
            gameConfig.current = gc.resolved || gameConfig.current;
          }
        } catch { /* ignore */ }
        ...
        // Update reviewer info in sidebar
  ...
  const bankCls = timeBankVal.value > STREAK_THRESHOLD * 2
    ? "tb-green" : timeBankVal.value > STREAK_THRESHOLD * 0.5
    ? "tb-yellow" : "tb-red";
  ...
  return (
    <div style={{ position: "fixed", inset: "0", zIndex: "50", background: "#0a0e14" }}>
      ...
      {streakBannerVisible.value && (
        <div class={`streak-banner ${streakBannerCls.value}`}>
          {streakBannerText.value}
        </div>
      )}

      {/* Progress bar */}
      <div id="progress-bar-container" style={{ gridColumn: "1/-1", gridRow: "1", height: "3px", background: "#1a1f2b" }}>
        <div id="progress-bar" style={{ height: "100%", background: "linear-gradient(90deg,#1f6feb,#8b5cf6)", width: `${progressPct.value}%`, transition: "width 0.4s ease", borderRadius: "0 2px 2px 0" }} />
      </div>

      {/* Review screen */}
      {view.value === "review" && currentItem.value && (
        <div id="review-screen" style={{ display: "grid", height: "100vh", gridTemplateColumns: "380px 1fr", gridTemplateRows: "3px 1fr 44px", overflow: "hidden" }}>
          {/* Left: Verdict panel */}
          <div id="verdict-panel" style={{ gridColumn: "1", gridRow: "2", display: "flex", flexDirection: "column", background: "#0f1219", borderRight: "1px solid #1a1f2b", overflowY: "auto" }}>
            <div id="verdict-content" style={{ padding: "24px 20px" }}>
              ...
              <div style={{ fontSize: "20px", fontWeight: "700", color: "#e6edf3", lineHeight: "1.3", marginBottom: "10px" }}>{currentItem.value.header}</div>

              {/* Verdict badge - review mode always "Bot answered NO" */}
              <div style={{ display: "inline-flex", alignItems: "center", gap: "6px", padding: "5px 12px", borderRadius: "20px", fontSize: "11px", fontWeight: "700", textTransform: "uppercase", letterSpacing: "0.8px", marginBottom: "20px", background: "rgba(248,81,73,0.12)", color: "#f85149", border: "1px solid rgba(248,81,73,0.2)" }}>
                <span style={{ display: "block", width: "6px", height: "6px", borderRadius: "50%", background: "#f85149" }} />
                Bot answered NO
              </div>

              <div style={{ fontSize: "14px", lineHeight: "1.6", color: "#9ca3af", marginBottom: "20px" }}>{currentItem.value.populated}</div>

              <div style={{ marginBottom: "16px" }}>
                <div style={{ fontSize: "10px", fontWeight: "700", textTransform: "uppercase", letterSpacing: "1.2px", color: "#58a6ff", marginBottom: "8px" }}>Defense</div>
                <div style={{ fontSize: "13px", lineHeight: "1.65", color: "#b0b8c4", padding: "12px 14px", background: "#141820", borderRadius: "10px", border: "1px solid #1a1f2b" }}>
                  {currentItem.value.defense || "No defense provided"}
                </div>
              </div>
              ...
              >
                ...
                <span style={{ marginLeft: "auto", fontSize: "10px", color: "#3d4452" }}>
                  <kbd style={{ background: "#141820", border: "1px solid #1e2736", borderRadius: "3px", padding: "0 4px", fontFamily: "monospace", fontSize: "10px", color: "#6e7681" }}>D</kbd>
                </span>
              </button>
              {thinkingOpen.value && (
                <div style={{ fontSize: "13px", lineHeight: "1.6", color: "#8b949e", padding: "10px 14px", background: "#141820", borderRadius: "10px", border: "1px solid #1a1f2b", fontStyle: "italic", marginBottom: "12px" }}>
                  {currentItem.value.thinking || "No reasoning provided"}
                </div>
              )}

              <div id="meta-row" style={{ display: "flex", gap: "6px", flexWrap: "wrap", marginTop: "8px", paddingTop: "16px", borderTop: "1px solid #1a1f2b" }}>
                <div class="meta-chip">Audit <strong>{currentItem.value.findingId}</strong></div>
                <div class="meta-chip">Q<strong>{currentItem.value.questionIndex}</strong></div>
                <div class="meta-chip">Left <strong>{peekItem.value ? "..." : "0"}</strong></div>
                {auditRemaining.value === 1 && (
                  <div class="meta-chip last-item"><strong>Final for audit</strong></div>
                )}
              </div>
            </div>
          </div>
          ...
          {/* Right: Transcript */}
          <div id="transcript-panel" style={{ gridColumn: "2", gridRow: "2", padding: "20px 24px", overflow: "hidden", minHeight: "0", position: "relative" }}>
            {searchOpen.value && (
                ...
                <span style={{ fontSize: "11px", color: "#484f58", whiteSpace: "nowrap" }}>
                  {searchMatchCount.value > 0
                    ? searchActiveIdx.value >= 0
                      ? `${searchActiveIdx.value + 1}/${searchMatchCount.value}`
                      : `${searchMatchCount.value} found`
                    : searchQuery.value ? "no match" : ""}
                </span>
                <button
                  style={{ background: "none", border: "none", color: "#484f58", cursor: "pointer", fontSize: "14px", padding: "2px 6px", borderRadius: "4px" }}
            ...
            <div id="transcript-body" style={{ columnGap: "24px", columnFill: "auto", overflowX: "scroll", overflowY: "hidden", scrollbarWidth: "none" }}>
          ...
          {/* Cheat sheet */}
          {cheatOpen.value && (
            <div id="cheat-sheet" style={{ display: "flex", gap: "20px", position: "fixed", bottom: "52px", left: "24px", zIndex: "2500", background: "#12161e", border: "1px solid #1e2736", borderRadius: "12px", padding: "16px 20px", boxShadow: "0 8px 32px rgba(0,0,0,0.5)" }}>
              <div style={{ display: "flex", flexDirection: "column", gap: "4px", minWidth: "130px" }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: "6px", fontSize: "11px", color: "#6e7681", padding: "2px 0" }}>
                  <kbd style={{ background: "#141820", border: "1px solid rgba(31,111,235,0.4)", borderRadius: "4px", padding: "0 5px", color: "#58a6ff", fontFamily: "monospace", fontSize: "10px" }}>Y</kbd>
                  <span>Confirm</span>
                </div>
                <div style={{ display: "flex", alignItems: "center", gap: "6px", fontSize: "11px", color: "#6e7681", padding: "2px 0" }}>
                  <kbd style={{ background: "#141820", border: "1px solid rgba(139,92,246,0.4)", borderRadius: "4px", padding: "0 5px", color: "#bc8cff", fontFamily: "monospace", fontSize: "10px" }}>N</kbd>
                  <span>Flip</span>
                </div>
              </div>
              <div style={{ display: "flex", flexDirection: "column", gap: "4px", minWidth: "130px" }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: "6px", fontSize: "11px", color: "#6e7681" }}><kbd style={{ background: "#141820", border: "1px solid #1e2736", borderRadius: "4px", padding: "0 5px", fontFamily: "monospace", fontSize: "10px", color: "#8b949e" }}>B</kbd><span>Undo</span></div>
                <div style={{ display: "flex", alignItems: "center", gap: "6px", fontSize: "11px", color: "#6e7681" }}><kbd style={{ background: "#141820", border: "1px solid #1e2736", borderRadius: "4px", padding: "0 5px", fontFamily: "monospace", fontSize: "10px", color: "#8b949e" }}>D</kbd><span>Toggle reasoning</span></div>
                <div style={{ display: "flex", alignItems: "center", gap: "6px", fontSize: "11px", color: "#6e7681" }}><kbd style={{ background: "#141820", border: "1px solid #1e2736", borderRadius: "4px", padding: "0 5px", fontFamily: "monospace", fontSize: "10px", color: "#8b949e" }}>H</kbd><kbd style={{ background: "#141820", border: "1px solid #1e2736", borderRadius: "4px", padding: "0 5px", fontFamily: "monospace", fontSize: "10px", color: "#8b949e" }}>L</kbd><span>Scroll cols</span></div>
              </div>
              <div style={{ display: "flex", flexDirection: "column", gap: "4px", minWidth: "130px" }}>
                <div style={{ fontSize: "9px", fontWeight: "700", textTransform: "uppercase", letterSpacing: "1px", color: "#3d4452", marginBottom: "2px", paddingBottom: "4px", borderBottom: "1px solid #1a1f2b" }}>Playback</div>
                <div style={{ display: "flex", alignItems: "center", gap: "6px", fontSize: "11px", color: "#6e7681" }}><kbd style={{ background: "#141820", border: "1px solid #1e2736", borderRadius: "4px", padding: "0 5px", fontFamily: "monospace", fontSize: "10px", color: "#8b949e" }}>P</kbd><span>Play / Pause</span></div>
                <div style={{ display: "flex", alignItems: "center", gap: "6px", fontSize: "11px", color: "#6e7681" }}><kbd style={{ background: "#141820", border: "1px solid #1e2736", borderRadius: "4px", padding: "0 5px", fontFamily: "monospace", fontSize: "10px", color: "#8b949e" }}>&larr;</kbd><kbd style={{ background: "#141820", border: "1px solid #1e2736", borderRadius: "4px", padding: "0 5px", fontFamily: "monospace", fontSize: "10px", color: "#8b949e" }}>&rarr;</kbd><span>Seek</span></div>
              </div>
              <div style={{ display: "flex", flexDirection: "column", gap: "4px", minWidth: "130px" }}>
                <div style={{ fontSize: "9px", fontWeight: "700", textTransform: "uppercase", letterSpacing: "1px", color: "#3d4452", marginBottom: "2px", paddingBottom: "4px", borderBottom: "1px solid #1a1f2b" }}>Search</div>
                <div style={{ display: "flex", alignItems: "center", gap: "6px", fontSize: "11px", color: "#6e7681" }}><kbd style={{ background: "#141820", border: "1px solid #1e2736", borderRadius: "4px", padding: "0 5px", fontFamily: "monospace", fontSize: "10px", color: "#8b949e" }}>/</kbd><span>Find</span></div>
                <div style={{ display: "flex", alignItems: "center", gap: "6px", fontSize: "11px", color: "#6e7681" }}><kbd style={{ background: "#141820", border: "1px solid #1e2736", borderRadius: "4px", padding: "0 5px", fontFamily: "monospace", fontSize: "10px", color: "#8b949e" }}>;</kbd><span>Next match</span></div>
              </div>
            </div>
          )}

          {/* Bottom bar */}
          <div id="bottom-bar" style={{ gridColumn: "1/-1", gridRow: "3", display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 24px", background: "#0f1219", borderTop: "1px solid #1a1f2b", height: "44px" }}>
            <button
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
              ...
              />
              <button
                style={{ width: "22px", height: "22px", borderRadius: "50%", border: "none", cursor: "pointer", background: "#1f6feb", color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", padding: "0" }}
                onClick={() => {
                  const a = audioRef.current;
              ...
              >&larr;5s</button>
              <div
                style={{ width: "140px", height: "4px", background: "#1a1f2b", borderRadius: "2px", cursor: "pointer", position: "relative" }}
                onClick={(e) => {
                  const a = audioRef.current;
                  if (!a) return;
              ...
              >
                <div style={{ height: "100%", background: "#58a6ff", borderRadius: "2px", width: `${audioFillPct.value}%`, pointerEvents: "none" }} />
              </div>
              <button
                style={{ background: "#141820", border: "1px solid #1e2736", borderRadius: "4px", color: "#6e7681", cursor: "pointer", fontSize: "10px", padding: "2px 6px", fontFamily: "monospace" }}
              ...
              {skipVisible.value && (
                <div style={{ display: "flex", alignItems: "center", gap: "5px" }}>
                  <span style={{ fontFamily: "monospace", fontSize: "9px", fontWeight: "700", color: "#8b5cf6", whiteSpace: "nowrap" }}>{skipLabel.value}</span>
                  <div style={{ width: "36px", height: "4px", background: "#1a1f2b", borderRadius: "2px", overflow: "hidden" }}>
                    <div style={{ height: "100%", width: `${skipBarPct.value}%`, background: "linear-gradient(90deg,#1f6feb,#8b5cf6)", borderRadius: "2px" }} />
                  </div>
                </div>
              )}
            </div>
            ...
            <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
              <span style={{ fontSize: "11px", fontWeight: "600", color: "#6e7681" }}>
                ...
                <span style={{ display: "inline-block", width: "40px", height: "3px", background: "#1a1f2b", borderRadius: "2px", overflow: "hidden", verticalAlign: "middle", margin: "0 4px" }}>
                  <span style={{ display: "block", height: "100%", background: "linear-gradient(90deg,#1f6feb,#8b5cf6)", borderRadius: "2px", width: `${xpBarPct.value}%` }} />
                </span>
              ...
              <a href="/review/dashboard" style={{ background: "none", border: "1px solid #1e2736", borderRadius: "6px", padding: "3px 10px", color: "#6e7681", fontSize: "10px", cursor: "pointer", textDecoration: "none", textTransform: "uppercase" }}>Dashboard</a>
              <button
        ...
        <div style={{ height: "100vh", display: "flex", alignItems: "center", justifyContent: "center", textAlign: "center" }}>
          <div>
            ...
            {summaryHtml.value
              ...
              : <p style={{ color: "#484f58", fontSize: "15px" }}>No items pending review. Check back later.</p>
            }
      ...
      {confirmOpen.value && (
        ...
        >
          <div style={{ background: "#12161e", border: "1px solid #1e2736", borderRadius: "16px", padding: "32px 36px", width: "400px", boxShadow: "0 8px 40px rgba(0,0,0,0.5)" }}>
            <h3 style={{ color: "#e6edf3", fontSize: "17px", fontWeight: "700", marginBottom: "8px" }}>Final Question for This Audit</h3>
            <p style={{ color: "#8b949e", fontSize: "13px", lineHeight: "1.5", marginBottom: "20px" }}>This is the last item for this audit. Submitting will finalize the review.</p>
            <div style={{ display: "inline-flex", alignItems: "center", gap: "6px", padding: "6px 12px", borderRadius: "8px", fontSize: "12px", fontWeight: "700", marginBottom: "16px", background: "rgba(139,92,246,0.1)", border: "1px solid rgba(139,92,246,0.2)", color: "#bc8cff" }}>
              {pendingDecision.value === "confirm" ? "Confirm fail" : "Flip to pass"}
            </div>
            ...
            <input
              ...
              onKeyDown={(e) => {
                if (e.key === "Enter" && confirmInput.value.trim().toUpperCase() === "YES" && pendingDecision.value) {
                  const dec = pendingDecision.value;
                  const rsn = pendingReason.value || undefined;
            ...
            <div style={{ display: "flex", gap: "8px", marginTop: "16px", justifyContent: "flex-end" }}>
              ...
              >Cancel</button>
              <button
                style={{ padding: "8px 20px", borderRadius: "8px", fontSize: "12px", fontWeight: "600", cursor: "pointer", background: "#8b5cf6", border: "none", color: "#fff", opacity: confirmInput.value.trim().toUpperCase() === "YES" ? 1 : 0.3 }}
...
const QUEUE_CSS = `
  ...
  .meta-chip.last-item { background: rgba(250,176,5,0.12); border-color: rgba(250,176,5,0.25); color: #fab005; animation: lastPulse 2s ease infinite; }
  ...
  .t-highlight { background: rgba(139,92,246,0.1); }
  ...
  .streak-banner { position: fixed; top: 18%; left: 50%; transform: translate(-50%,0) scale(1); font-size: 28px; font-weight: 900; text-transform: uppercase; letter-spacing: 3px; z-index: 2000; pointer-events: none; text-shadow: 0 0 30px currentColor; padding: 12px 32px; border-radius: 12px; background: rgba(11,15,21,0.65); backdrop-filter: blur(12px); animation: streakIn 0.15s ease; }
  .s-double { color: #58a6ff; }
  ...
  .s-godlike { color: #fbbf24; font-size: 50px; text-shadow: 0 0 40px #fbbf24, 0 0 80px rgba(251,191,36,0.4); }
  @keyframes streakIn { from { opacity: 0; transform: translate(-50%,0) scale(0.5); } to { opacity: 1; transform: translate(-50%,0) scale(1); } }

...
- Question text (`template` field -- not a separate object, it's a field on AuditQuestion)
- `expectedAnswer` -- the expected answer for this question
- `questionType` -- references a QuestionType name (enum of all available types)
  - Each QuestionType defines: name, regex (validation), parser key ("bool" | "int" | "float" | "string"), schema (JSON Schema representation of the type's value)
- Scoring weight

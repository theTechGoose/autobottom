...
| Backfill Chargeback Entries | Data Maintenance modal → "Backfill Scores" tab | Done — admin tool to repair the chargeback/wire "payroll" sheet: re-reads every entry whose audit completed in a From/To window vs the finding's CURRENT answers, **removing** now-passing rows / **rewriting** still-failing ones (idempotent). **Chunked + self-polling** like the Cleanup sweep (kickoff `chargeback-backfill-start` → `ChargebackBackfillProgress` ticks `chargeback-backfill-tick` 100 fids/request, backend reconciles ~20-concurrent) so it never hits the Deno Deploy request timeout; live progress bar with removed/rewritten counts. Backend `POST /admin/chargeback-backfill-list` + `/chargeback-backfill-process`; logs under `[CB-BACKFILL]`. |

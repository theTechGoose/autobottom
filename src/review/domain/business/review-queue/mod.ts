...
async function abandonAuditSessionWithData(
  ...
  for (const { key, value } of decided) {
    ...
    const baseItem: ReviewItem = {
      ...
      thinking: asAnswerText(value.thinking),
      defense: asAnswerText(value.defense),
...
async function rehydrateItemFromFinding(orgId: OrgId, item: ReviewItem): Promise<ReviewItem> {
  ...
  try {
    ...
    const filled: ReviewItem = {
      ...
      thinking: asAnswerText(item.thinking) || asAnswerText(q?.thinking),
      defense: asAnswerText(item.defense) || asAnswerText(q?.defense),
  ...
  for (const [fid, decisions] of myDecidedByFinding) {
    ...
    for (const d of decisions) {
      ...
      const baseItem: ReviewItem = {
        ...
        thinking: asAnswerText(d.thinking),
        defense: asAnswerText(d.defense),
        answer: d.answer ?? "No",
...
export async function undoDecision(
  ...
  let item: ReviewItem = {
    ...
    thinking: asAnswerText(decided.thinking),
    defense: asAnswerText(decided.defense),

...
cleanup() {
  ...
  git add frontend/ main.ts tests/e2e/ tools/ docs/ 2>/dev/null
}
trap cleanup EXIT

# Save .gitignore, hide frontend + root-level files + E2E tests + docs from shape-checker.
# docs/ holds plain markdown (context.md, CHANGELOG.md) that doesn't fit the src/
# module spec — same category as tools/ and main.ts.
...
git rm -rf --cached --quiet docs/ 2>/dev/null

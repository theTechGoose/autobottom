...
export default define.page(async function AdminAuditsPage(ctx) {
  ...
  return (
    <Layout title="Audit History" section="admin" user={user} pathname={url.pathname} hideSidebar>
      ...
      {/* Appeal detail — plain modal shell (NOT an island), opened by the
          APPEAL badge in the table fragment. Lives on the page so it survives
          every HTMX table swap. Same shell the manager and operations audit
          histories use, filled by the same /api/manager/appeal fragment. */}

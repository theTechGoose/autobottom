...
/** Split a possibly comma-joined address string into validated, de-duped
...
/** Noun+verb half of the appeal-result overturn line, paired in the template
 *  with "{{overturns}} of {{totalQuestions}}". `totalQuestions` on a judge
 *  payload is the number of questions in THAT APPEAL, not the audit's question
 *  count — "7 of 7 questions were overturned" on a 25-question audit read as if
 *  the whole audit were 7 questions. Saying "appealed questions" makes the scope
 *  explicit. Agreement follows totalQuestions so a one-question appeal doesn't
 *  render "1 of 1 appealed questions were overturned". */
export function overturnPhraseFor(totalQuestions: number): string {
  return totalQuestions === 1
    ? "appealed question was overturned"
    : "appealed questions were overturned";
}
...
async function sendAuditCompleteEmail(orgId: OrgId, payload: AuditCompletePayload): Promise<void> {
  ...
  for (const { source, value } of chain) {
    if (isEmailAddr(value)) { to = value; toSource = source; break; }
  }
...
const DEFAULT_REAUDIT_RECEIPT_TEMPLATE: EmailTemplate = {
  ...
  createdAt: 0,
  updatedAt: 0,
  html: `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>Recording Submitted</title></head><body style="margin:0;padding:0;background:#070d18;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;"><table width="100%" cellpadding="0" cellspacing="0" style="background:#070d18;min-height:100vh;"><tr><td align="center" style="padding:40px 16px;"><table width="580" cellpadding="0" cellspacing="0" style="max-width:100%;width:580px;background:#0d1520;border:1px solid #1e2d45;border-radius:16px;overflow:hidden;"><tr><td style="padding:28px 32px 24px;text-align:center;border-bottom:1px solid #1a2840;"><div style="margin-bottom:12px;"><span style="font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:1.5px;color:#3b82f6;">{{appealTypeLabel}}</span></div><h1 style="margin:0 0 10px;font-size:24px;font-weight:700;color:#e6edf3;line-height:1.2;">Re-Audit Submitted</h1><p style="margin:0;font-size:14px;color:#8b949e;">Hey <strong style="color:#c9d1d9;">{{teamMemberFirst}}</strong>, your results will be available shortly.</p></td></tr><tr><td style="padding:24px 32px;"><table width="100%" cellpadding="0" cellspacing="0" style="background:#111d2e;border:1px solid #1e2d45;border-radius:10px;overflow:hidden;"><tr><td style="padding:16px 20px;text-align:center;border-bottom:1px solid #1a2840;"><div style="font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:1.5px;color:#4a6080;margin-bottom:6px;">Record</div><div style="font-size:20px;font-weight:700;color:#e6edf3;">{{recordId}}</div></td></tr><tr><td style="padding:16px 20px;text-align:center;border-bottom:1px solid #1a2840;"><div style="font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:1.5px;color:#4a6080;margin-bottom:6px;">Original Genie</div><div style="font-size:16px;font-weight:600;color:#3b82f6;">{{originalGenieId}}</div></td></tr><tr><td style="padding:16px 20px;text-align:center;border-bottom:1px solid #1a2840;"><div style="font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:1.5px;color:#4a6080;margin-bottom:6px;">New Genie(s)</div><div style="font-size:16px;font-weight:600;color:#e6edf3;">{{newGenieIds}}</div></td></tr><tr><td style="padding:16px 20px;text-align:center;"><div style="font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:1.5px;color:#4a6080;margin-bottom:6px;">Submitted</div><div style="font-size:14px;font-weight:500;color:#c9d1d9;">{{submittedAt}}</div></td></tr></table></td></tr>{{commentBlock}}<tr><td style="padding:0 32px 32px;text-align:center;"><a href="{{reportUrl}}" style="display:inline-block;padding:12px 32px;background:#3b82f6;color:#ffffff;text-decoration:none;border-radius:8px;font-size:14px;font-weight:600;border:1px solid #2563eb;">View Audit Report</a></td></tr><tr><td style="background:#080f1a;border-top:1px solid #1a2840;padding:14px 28px;text-align:center;"><p style="margin:0;font-size:10px;color:#3a5070;font-family:monospace;">Audit ID: {{findingId}}</p></td></tr></table></td></tr></table></body></html>`,
};

/** The team member's typed note for a re-audit submission, as a table row that
 *  drops into the re-audit-receipt email's outer layout table. Returns "" when
 *  no comment was typed, so the section simply doesn't appear.
 *
 *  This exists because the appeal modal pre-fills the genie box with the
 *  ORIGINAL genie ID — so a member who types the replacement number into the
 *  comment box instead of the genie box resubmits the same recording. Showing
 *  the note back to them next to "New Genie(s)" makes that mismatch obvious.
 *  Exported for testing. */
export function renderReAuditCommentBlock(comment: string): string {
  const text = String(comment ?? "").trim();
  if (!text) return "";
  return `<tr><td style="padding:0 32px 24px;"><div style="background:#111d2e;border:1px solid #1e2d45;border-left:3px solid #3b82f6;border-radius:10px;padding:16px 20px;"><div style="font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:1.5px;color:#4a6080;margin-bottom:8px;">Your Note</div><div style="font-size:14px;color:#c9d1d9;line-height:1.6;white-space:pre-wrap;">${escHtml(text)}</div></div></td></tr>`;
}
...
async function sendReAuditReceiptEmail(orgId: OrgId, payload: ReAuditReceiptPayload): Promise<void> {
  ...
  const vars: Record<string, string> = {
    ...
    comment: payload.comment ?? "",
    commentBlock: renderReAuditCommentBlock(payload.comment ?? ""),
...
async function sendAppealDecidedEmail(orgId: OrgId, payload: JudgeDecisionPayload): Promise<void> {
  ...
  const vars: Record<string, string> = {
    ...
    overturnPhrase: overturnPhraseFor(payload.totalQuestions ?? 0),
...
/** Env var that turns remediation emails ON. Absent ⇒ OFF.
 *
 *  Off is the safe default and it is deliberately the ABSENCE of the var: a
 *  fresh deploy, a new preview branch, or a wiped env all land paused rather
 *  than mailing real team members. Going live is one explicit action — set
 *  this to true in the Deno Deploy env.
 *
 *  It is an env switch rather than webhook config because the manager webhook
 *  config has no enable flag (fireWebhook always calls whichever email handler
 *  is registered) and its `testEmail` only REDIRECTS the mail, never stops it. */
const MANAGER_REVIEW_EMAIL_ENV = "MANAGER_REMEDIATION_EMAILS_ENABLED";

/** Truthiness for an on/off env var. Exported for tests.
 *
 *  Only an explicit affirmative enables sending — anything unset, empty, or
 *  unrecognised (including a fat-fingered "ture") reads as OFF. For a switch
 *  whose failure mode is emailing real people, ambiguity must not mean on. */
export function envFlagEnabled(raw: string | undefined): boolean {
  return ["true", "1", "yes", "on"].includes(String(raw ?? "").trim().toLowerCase());
}

/** Read at call time, not module load, so flipping the var takes effect on the
 *  next isolate without waiting for a code deploy. */
function managerReviewEmailsEnabled(): boolean {
  return envFlagEnabled(Deno.env.get(MANAGER_REVIEW_EMAIL_ENV));
}

async function sendManagerReviewEmail(orgId: OrgId, payload: ManagerReviewPayload): Promise<void> {
  if (!managerReviewEmailsEnabled()) {
    console.log(
      `🔇 [WEBHOOK:manager] remediation email PAUSED (${MANAGER_REVIEW_EMAIL_ENV} not enabled) — not sending fid=${String(payload.findingId ?? payload.finding?.id ?? "")}`,
    );
    return;
  }

...
Deno.test({
  ...
  async fn() {
    await withKv(async () => {
      ...
      const updated = await getFinding("org1", "finding-invalid-id");
...
Deno.test({
  ...
  async fn() {
    await withKv(async () => {
      ...
      const updated = await getFinding("org1", "finding-happy-init");

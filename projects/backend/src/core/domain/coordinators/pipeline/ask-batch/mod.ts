...
async function askLlmOne(
  ...
  // Check cache
  const cached = await kvInst.getCachedAnswer(orgId, findingId, question.populated);
  ...
  if (ast.length === 0 || (ast.length === 1 && ast[0].length === 1 && !ast[0][0].flip)) {
    ...
    await kvInst.cacheAnswer(orgId, findingId, question.populated, answer);
  ...
  for (const andNodes of ast) {
    ...
    for (const node of andNodes) {
      ...
      if (boolAnswer === null) {
        ...
        await kvInst.cacheAnswer(orgId, findingId, question.populated, fallbackAnswer);
  ...
  await kvInst.cacheAnswer(orgId, findingId, question.populated, finalAnswer);
...
export async function stepAskBatch(req: Request): Promise<Response> {
  ...
  kv.trackActive(orgId, findingId, `ask-batch-${batchIndex}`).catch(() => {});
  ...
  const allPopulated = await kv.getPopulatedQuestions(orgId, findingId) ?? finding.populatedQuestions ?? [];
  ...
  for (const q of questions) {
    try {
      const answer = await askLlmOne(kv, q, orgId, findingId);
      answers.push(answer);
    } catch (err: any) {
  ...
  // Save batch answers
  await kv.saveBatchAnswers(orgId, findingId, batchIndex, answers);
  console.log(`[STEP-ASK] ${findingId}: Batch ${batchIndex} done (${answers.length} answers)`);

  // Fan-in: decrement counter
  const remaining = await kv.decrementBatchCounter(orgId, findingId);

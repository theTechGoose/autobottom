...
import { saveFinding, getFinding, cacheQuestions } from "../../../data/kv/mod.ts";
...
function withMockFetch(fn: () => Promise<void>): Promise<void> {
  ...
  mockFetchJson(/pinecone\.io\/vectors/, { upsertedCount: 1 });
  // Mock Pinecone describe_index_stats
  mockFetchJson(/pinecone\.io\/describe/, { namespaces: {} });
  // Mock QuickBase questions query
  mockFetchJson("api.quickbase.com", { data: [] });
...
Deno.test({
  ...
  async fn() {
    await withKv(async () => {
      ...
      await cacheQuestions("org1", "dest-cached", cachedQuestions);
      ...
      const updated = await getFinding("org1", "finding-cached-qs");

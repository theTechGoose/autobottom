...
export async function stepTranscribeCb(req: Request): Promise<Response> {
  ...
  (await Kv.getInstance()).trackActive(orgId, findingId, "diarize").catch(() => {});
  ...
  await (await Kv.getInstance()).saveTranscript(orgId, findingId, finding.rawTranscript, finding.diarizedTranscript);

  await (await Kv.getInstance()).saveFinding(orgId, finding);

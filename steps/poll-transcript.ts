...
export async function stepPollTranscript(req: Request): Promise<Response> {
  ...
  try {
    ...
    console.warn(`[STEP-POLL-TRANSCRIPT] ${findingId}: Poll request failed, retrying in ${POLL_DELAY_SECONDS}s:`, err);
  ...
  if (transcript.status === "queued" || transcript.status === "processing") {
    console.log(`[STEP-POLL-TRANSCRIPT] ${findingId}: Status=${transcript.status}, re-polling in ${POLL_DELAY_SECONDS}s`);
    await enqueueStep("poll-transcript", { findingId, orgId }, POLL_DELAY_SECONDS);
  ...
  if (transcript.status !== "completed") {
    console.error(`[STEP-POLL-TRANSCRIPT] ${findingId}: Terminal status=${transcript.status} error=${transcript.error}`);
    finding.rawTranscript = "Genie Invalid";
    finding.findingStatus = "finished";
    await saveFinding(orgId, finding);
    await enqueueStep("transcribe-complete", { findingId, orgId });
  ...
  await saveFinding(orgId, finding);
  await enqueueStep("transcribe-complete", { findingId, orgId });
  console.log(`[STEP-POLL-TRANSCRIPT] ${findingId}: Done, transcript length=${result.text.length}`);

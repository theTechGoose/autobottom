...
**Model commentary stored as the transcript (`4oL3fw_Coxvzpx7El_qip` incident, fixed 2026-07-21):** the diarization reply was a markdown critique — a `| # | Original line | Issue identified |` table, a "Summary of problems", a `## Corrected transcription` heading with the real transcript inside a code fence, then a "what was changed" changelog. It was stored as `audit-transcript.diarized` and a reviewer graded off it two minutes later. Four compounding causes, all fixed:

- **The repair loop had become a critique dialogue.** `diarize()` kept ONE growing conversation, pushing each attempt as an `assistant` turn and the manager bot's feedback as a `user` turn — so by attempt 2 the model was answering a critique, not transcribing a call. **Rule: every attempt is a fresh two-message conversation**, with the output contract re-asserted and feedback folded into the user message. Never accumulate assistant turns.
- **The validator only knew refusals.** The critique passed every check trivially — it *quotes* the labels dozens of times, has no refusal phrasing, and commentary made it *longer* than the raw. [diarization-validation/mod.ts](src/core/business/diarization-validation/mod.ts) now also rejects on markdown structure + commentary phrasings (`looksLikeCommentary`) and a **labeled-line-ratio floor** — a transcript must actually *be* speaker turns, not merely mention them.
- **A free-text QA "Yes" was the gate.** The reply closed by asserting it satisfied the required format — exactly what talks a QA bot into approving — and that short-circuited past a clean attempt already held in `firstValid`. A QA pass now only counts when the output classifies `clean`.
- **Three "independent" guards were one predicate voting three times, and five readers had none.** The review queue, judge queue, manager remediation, evidence excerpts, and question lab all read `diarized` unfiltered. **Rule: every human-facing transcript read goes through `safeDiarized(diarized, raw)`**, so stored-bad rows render clean even before any repair runs.

Rather than discard a contaminated reply, `extractDiarizedTranscript` **cuts the real transcript back out**: `clean` → lift from a code fence (`fenced`) → drop every non-turn line (`filtered`), each candidate gated on **bidirectional word-overlap fidelity** against the raw transcript (precision catches invention, recall catches condensation). A scan of 1,092 production audits found 3, one per shape — all are regression fixtures in [diarization-validation/test.ts](src/core/business/diarization-validation/test.ts). The third is why the fidelity gate exists: the model wrote a how-to essay on speaker labelling whose *example* fence held a fabricated conversation, and structure alone would have stored that invention as the record of a real call. **Grading was never affected — `step-ask-all` reads `rawTranscript`.**
...
### Chargeback / payroll sheet — stays in sync on flips + repair tool

The weekly **chargeback / wire ("payroll") sheet** is built from per-finding `chargeback-entry` (internal date-leg) and `wire-deduction-entry` (partner package) records, filtered to reviewed findings and exported to Google Sheets by the `weekly-sheets` cron (Mondays 13:00 UTC). The bot writes those entries at `step-finalize` from **its own** grade — so any later human flip must keep them in sync or the sheet shows a stale failure.

- **Sync on every human flip** — `finalizeReviewedAudit` (reviewer queue) and `adminFlipQuestion` (pencil-flip / Bulk Flip) call the shared `syncChargebackWireToScore(orgId, findingId, finding, score)` in [review-queue/mod.ts](src/review/domain/business/review-queue/mod.ts). It recomputes from the **final** answers: a now-passing audit's entry is **deleted**, a still-failing one is **rewritten** to the reviewed score + remaining fails, and a Yes→No flip that introduces a fail **creates** one. Date-leg → chargeback entry, package → wire entry (deleted symmetrically on pass — not left as a hidden 100% row); the entry `ts` keeps the original `completedAt` so the deduction stays in the same pay period. **Canonical predicate:** a chargeable fail is an explicit `=== "No"` *with a header* (matching step-finalize), and `passing = score >= 100 || no "No" headers` — so N/A / Error / blank answers never count and the score can't disagree with the fail set. **Without this**, a reviewed-to-100% audit kept its pre-review chargeback row and rode the weekly export onto the sheet as a fail (the `483830` / ACT incident, 2026-06-30).
- **Repair the existing sheet** — **Data Maintenance → Backfill Scores → "Backfill Chargeback Entries"**. Re-reads every chargeback + wire entry whose audit completed in a From/To window against the finding's *current* answers, then removes/rewrites the stale ones. Idempotent. It's **chunked + self-polling** (mirrors the Cleanup sweep) so it survives the Deno Deploy request timeout: `POST /admin/chargeback-backfill-list` enumerates the fids once (cheap index reads), then a progress fragment ticks `POST /admin/chargeback-backfill-process` 100 fids/request, reconciling them **~20-concurrent** (`reconcileChargebackForFinding`; `Promise.allSettled` so one failed save can't abort the run). A ~3000-entry window finishes in ~2 min; every step logs under `[CB-BACKFILL]`. After it runs, re-run the weekly export to refresh the Google Sheet. Backend in [judge-repository/mod.ts](src/judge/domain/data/judge-repository/mod.ts); frontend kickoff/tick/progress under [routes/api/admin/modal/maintenance/](frontend/routes/api/admin/modal/maintenance/).
...
### Transcript Repair (Data Maintenance)

Finds audits whose stored `audit-transcript.diarized` is model **output** rather than speech — a refusal (`76UGB0…`) or a markdown critique wrapping the real transcript (`4oL3fw…`). Deliberately **two buttons, so counting is never coupled to changing**:

- **Scan** — classifies only, **zero writes**. Answers "how many are impacted?" Safe to run as wide as you like.
- **Repair** — rendered *only on the scan's result card*, beside the impacted-audit table (flagging any a human already reviewed) and a sample of what is actually stored. So you cannot repair without having looked first.

Same chunked list-then-tick shape as the chargeback backfill: `POST /admin/transcript-repair-list` enumerates the window from `audit-done-idx` once (indexed, deduped to newest row per audit), then a progress fragment ticks `POST /admin/transcript-repair-process` 100 fids/request, classifying **~20-concurrent** (`Promise.allSettled`). Anything that isn't an explicit `mode: "repair"` is coerced to a dry scan at *both* the route and the frontend, so a dropped field can never become a write. Idempotent — a repaired row classifies `clean` next pass. Logs under `[TRANSCRIPT-REPAIR]`.

**It writes the transcript doc ONLY, never the finding** — `saveFinding` is a full-document overwrite and racing it is what lost `BofFRUvr…` (above). Legacy findings carrying `diarizedTranscript` inline are covered by the read-side `safeDiarized` guard instead. Backend: [transcript-repair/mod.ts](src/audit/domain/business/transcript-repair/mod.ts); frontend kickoff/tick/progress under [routes/api/admin/modal/maintenance/](frontend/routes/api/admin/modal/maintenance/).
